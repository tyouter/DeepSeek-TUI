...
## Coding Protocol（编码过程管控）— v1.2 新增

> 来源: 融合 Andrej Karpathy 的 CLAUDE.md 实践模式 + Harness 编程范式。
> 管控的不是"代码写得好不好"（那是 Phase 2 安全审查的事），而是
> "编码会话本身的结构和节奏"——什么时候读代码、什么时候写、什么时候停。
...
**核心思想: AI 不是自由创作，而是在预设合约（harness）内实现。**

```
        ┌─────────────────────────────────────┐
        │          Harness（合约层）            │
        │  ┌─────────┐ ┌──────┐ ┌──────────┐  │
        │  │ 类型定义  │ │ Trait │ │ 测试用例  │  │  ← 人+AI 共同设计
        │  └─────────┘ └──────┘ └──────────┘  │
        │              ┌──────────┐            │
        │              │ 不变量    │            │
        │              └──────────┘            │
        └─────────────────────────────────────┘
                         ...
                         ▼
        ┌─────────────────────────────────────┐
        │         AI 实现层                    │  ← AI 在 harness 内实现
        │  fn implement_thing() -> Result {    │
        │      // AI fills this in             │
        │  }                                   │
        └─────────────────────────────────────┘
                         │
...
| 层 | 谁负责 | 产出 | 验证方式 |
|----|--------|------|----------|
| **L1: 类型定义** | AI 设计 → 用户确认 | struct/enum/trait 签名 | cargo check 通过 |
| **L2: 测试用例** | AI 编写（先于实现） | 测试函数，覆盖 happy path + edge case | `cargo test` 失败（预期） |
| **L3: 不变量** | AI 声明 | `debug_assert!`、invariant 注释、doc test | 运行时断言 |
| **L4: 实现** | AI 填充 | 函数体 | 所有测试通过 |

**Harness 工作流：**

```
Step 1: 设计合约
  ├── 定义类型（struct, enum, trait）
  ├── 定义函数签名（输入、输出、错误类型）
  ├── 编写测试用例（此时实现为空）
  └── → 用户确认合约
...
| 场景 | 是否用 Harness |
|------|---------------|
| 新增模块 / 新增 public API | **必须用** — 先定义 trait/类型，再实现 |
| 修改现有函数内部逻辑 | 部分用 — 先补测试（如果缺失），再修改 |
| 纯 Bug 修复（改一行） | 可简化 — 但必须有回归测试 |
| UI 渲染改动 | 部分用 — 合约是"渲染后的帧应该长什么样"（PTY test） |

### C.3 编码会话结构

每次编码会话（从一个 WBS 子任务开始到 commit）遵循固定节奏：
...
**Phase C 循环规则：**
- 每次循环是 1 个函数或 1 个紧密相关的变更组
- 如果 cargo check 连续 2 次失败：停下来，重新理解错误，不要盲目修改
- 如果 cargo test 连续 3 次失败：检查测试本身是否正确，是否需要调整合约
- 每个循环的 commit message 描述这个增量做了什么

### C.4 上下文工程（Context Engineering）
...
| 反模式 | 为什么危险 | 正确做法 |
|--------|-----------|----------|
| **探索性编码** — 不读代码，直接写 patch | 不理解架构，80% 会出错 | Phase A 上下文准备 |
| **大水漫灌** — 一次改 8 个文件 | 编译失败时不知道哪个改动是原因 | Phase C 增量循环，每次 1 个 commit |
| **跳过测试** — "这个太简单了，不需要测试" | 没有回归保护，未来改动可能破坏 | Harness L2: 先写测试 |
| **拷贝粘贴** — 从 StackOverflow/其他 crate 复制代码 | 引入不安全的模式、不匹配的风格 | 理解后用自己的方式写，匹配项目风格 |
| **静默修改** — 改代码不告诉用户为什么 | 用户失去对代码库的理解 | 每个 commit 说明 what + why |
| **死循环修复** — 同一错误反复修改反复失败 | 浪费 token，方向错误 | Phase C 规则：2 次失败就停下来 |
